...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchUserCategories() {
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/custom-categories?username=${username}&include_public=false`
      );
  ...
  window.deleteCategory = async function (categoryId, categoryName) {
    if (
      !confirm(
        `Are you sure you want to delete the category "${categoryName}"? This will also delete all questions in this category.`
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/custom-categories/${categoryId}?username=${username}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        showNotification(
          `Category deleted successfully! ${data.questions_deleted} questions were also deleted.`,
          "success"
        );
