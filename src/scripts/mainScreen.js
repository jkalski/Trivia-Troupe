...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.category').appendChild(createCategoryButton);
    ...
    async function fetchCustomCategories() {
        try {
            const response = await fetch(`http://127.0.0.1:5000/custom-categories?username=${username}&include_public=true`);
            ...
            if (customCategories.length > 0) {
                ...
                document.querySelector('.container').appendChild(customCategoriesWrapper);
            }
        } catch (error) {
